...
make -j ${cpuNumber} fift validator-engine lite-client pow-miner validator-engine-console generate-random-id dht-server func toslibjson rldp-http-proxy
systemctl restart validator

# End
echo -e "${COLOR}[1/1]${ENDC} TOS components update completed"
exit 0

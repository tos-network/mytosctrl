...
def TestMoveCoins(wallet, dest, coins, **kwargs):
	start = time.time()
	tos.MoveCoins(wallet, dest, coins, timeout=600, **kwargs)
	end = time.time()
	diff = int(end - start)
...
def Init():
	...
	wallet1 = tos.CreateWallet(f"test{vnum+1}", workchain=0, version="v1")
	wallet2 = tos.CreateWallet(f"test{vnum+2}", workchain=0, version="v2")
	wallet3 = tos.CreateWallet(f"test{vnum+3}", workchain=0, version="v3")
	local.buffer["vnum"] += 3
	tos.ActivateWallet(wallet1)
	tos.ActivateWallet(wallet2)
	tos.ActivateWallet(wallet3)

...
class LiteClient:
	...
	def Run(self, cmd, **kwargs):
		...
		validatorStatus = self.tos.GetValidatorStatus()
...
class MyTosCore():
	...
	def Refresh(self):
		...
		if liteClient is not None:
			self.liteClient.tos = self # magic
	...
	def GetInitBlock_new(self):
		...
		statesDir = "/var/tos-work/db/archive/states"
		os.chdir(statesDir)
	...
	def GetOffers(self):
		...
		for offer in rawOffers:
			...
			# Create dict
			# parser from: https://github.com/tos-network/tos/blob/dab7ee3f9794db5a6d32c895dbc2564f681d9126/crypto/smartcont/config-code.fc#L607
			item = dict()
			item["config"] = dict()
			item["hash"] = hash
	...
	def GetComplaints(self, electionId=None, past=False):
		...
		for complaint in rawComplaints:
			...
			# Create dict
			# parser from: https://github.com/tos-network/tos/blob/dab7ee3f9794db5a6d32c895dbc2564f681d9126/crypto/smartcont/elector-code.fc#L1149
			item = dict()
	...
	def DownloadContract(self, url, branch=None):
		...
		#	file.write("/usr/bin/tos/crypto/func $@")
		#	file.close()
		#end if
	...
	def CreatePool(self, poolName, validatorRewardSharePercent, maxNominatorsCount, minValidatorStake, minNominatorStake):
		...
		if not os.path.isdir(contractPath):
			self.DownloadContract("https://github.com/tos-network/nominator-pool")
		#end if
...
class TosBlocksScanner():
	def __init__(self, tos, **kwargs):
		self.tos = tos
	...
	def StatusReading(self):
		while True:
			validatorStatus = self.tos.GetValidatorStatus()
			validatorOutOfSync = validatorStatus.get("outOfSync")
			if self.tos.liteClient.pubkeyPath is None:
				self.working = False
				self.closing = True
				text = "TosBlocksScanner error: local liteserver is not configured, stop thread."
				self.AddLog(text, "error")
				exit()
			if validatorOutOfSync > 20:
				self.working = False
				text = f"TosBlocksScanner warning: local liteserver is out of sync: {validatorOutOfSync}."
	...
	def ScanBlock(self):
		block = self.Try(self.tos.GetLastBlock)
	...
	def ReadBlock(self, block):
		...
		shards = self.Try(self.tos.GetShards, args=(block,))
		for shard in shards:
	...
	def SearchBlock(self, workchain, shardchain, seqno):
		...
		block = self.Try(self.tos.GetBlock, args=(workchain, shardchain, seqno))
		self.StartWithMode(self.NewBlockReaction, args=(block,))
	#end define
	...
	def NewBlockReaction(self, block):
		...
		transactions = self.Try(self.tos.GetTransactions, args=(block,))
	...
	def NewTransReaction(self, trans):
		...
		messageList = self.Try(self.tos.GetTrans, args=(trans,))
...
def EnableVcEvent():
	...
	tos = MyTosCore()
	wallet = tos.CreateWallet("validator_wallet_001", -1)
	local.db["validatorWalletName"] = wallet.name

	adnlAddr = tos.CreateNewKey()
	tos.AddAdnlAddrToValidator(adnlAddr)
...
def Complaints(tos):
	...
	config32 = tos.GetConfig32()
	electionId = config32.get("startWorkTime")
	complaintsHashes = tos.SaveComplaints(electionId)
	complaints = tos.GetComplaints(electionId)
	for key, item in complaints.items():
...
def Slashing(tos):
	...
	if slashTime != start:
		...
		tos.CheckValidators(start, end)
		local.buffer["slashTime"] = start
#end define

def ScanLiteServers(tos):

	filePath = tos.liteClient.configPath
	...
	for index in range(len(liteservers)):
		try:
			tos.liteClient.Run("last", index=index)
			result.append(index)
		except: pass
	#end for
...
def General():
	...
	tos = MyTosCore()
	scanner = TosBlocksScanner(tos, local=local)
	#scanner.Run()

	local.StartCycle(Elections, sec=600, args=(tos, ))
	local.StartCycle(Statistics, sec=10, args=(scanner,))
	local.StartCycle(Offers, sec=600, args=(tos, ))
	local.StartCycle(Complaints, sec=600, args=(tos, ))
	local.StartCycle(Slashing, sec=600, args=(tos, ))
	local.StartCycle(Domains, sec=600, args=(tos, ))
	local.StartCycle(ScanLiteServers, sec=60, args=(tos,))
	Sleep()
#end define
